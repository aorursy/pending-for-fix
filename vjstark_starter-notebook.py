...
print("Model: Collaborative Filtering with Cosine Similarity (j=10)")
collab_cos10 = collaborative_filter(s_train, s_input, sim_fn=jaccard_sim, j=10, k=k_top)
print("MAP = {0:.3f}".format(mapk_score(s_hidden, get_recs(collab_cos10), k=k_top)))
print("UHR = {0:.3f}".format(uhr_score(s_hidden, get_recs(collab_cos10), k=k_top)))
...
content_cos10 = content_filter(s_items, s_input, sim_fn=cosine_sim, k=k_top)
print("MAP = {0:.3f}".format(mapk_score(s_hidden, get_recs(content_cos10), k=k_top)))
...
print("Model: Content Filtering with Cosine Similarity (j=10)")
svd_cos10 = svd_filter(s_train, s_input, n_factors=2, baselines=None, threshold=0.01, k=10)
print("MAP = {0:.3f}".format(mapk_score(s_hidden, get_recs(svd_cos10), k=k_top)))
...
final_scores = collaborative_filter(s_train, s_all_input, sim_fn=simpson_sim, j=40)
