...
# Process training and validation data to make it ready for fitting.
...
validation_generator = val_datagen.flow_from_directory(validation_data_dir,target_size=(img_size, img_size),
                                                        batch_size=batch_size,class_mode='categorical',shuffle=True) 
# Process testing data and make it ready for predictions.
...
vgg_all_model= Sequential()
vgg_all_model.add(base_model)
vgg_all_model.add(Flatten())
vgg_all_model.add(Dense(4096, activation='relu'))
vgg_all_model.add(Dropout(0.5))
vgg_all_model.add(Dense(4096, activation='relu'))
vgg_all_model.add(Dropout(0.5))
vgg_all_model.add(Dense(2, activation='softmax'))
vgg_all_model.layers[0].trainable = True
print ('Summary of the model...')
vgg_all_model.summary()
print ('Compiling model...')
vgg_all_model.compile(optimizer='sgd', loss='categorical_crossentropy', metrics=['accuracy'])
#vgg_all_model.compile(optimizer=SGD(lr=1e-3), loss='categorical_crossentropy', metrics=['accuracy'])
vgg_all_model.compile(optimizer=SGD(lr=1e-3, decay=1e-6, momentum=0.9, nesterov=True), loss='categorical_crossentropy', metrics=['accuracy'])
...
checkpoint = ModelCheckpoint('vgg_all_model.h5',monitor='val_loss', verbose=1, save_best_only=True,mode='auto')
...
fitted_vgg_all_model=vgg_all_model.fit_generator(train_generator,
...
acc = fitted_vgg_all_model.history['acc']
val_acc = fitted_vgg_all_model.history['val_acc']
loss = fitted_vgg_all_model.history['loss']
val_loss =fitted_vgg_all_model.history['val_loss']
...
pred=vgg_all_model.predict_generator(test_generator,steps=math.ceil(test_generator.samples/test_generator.batch_size),verbose=1)
