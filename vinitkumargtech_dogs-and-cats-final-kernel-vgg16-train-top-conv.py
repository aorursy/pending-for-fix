...
vgg_conv_model = Sequential()
vgg_conv_model.add(base_model)
vgg_conv_model.add(Flatten())
vgg_conv_model.add(Dense(4096, activation='relu'))
vgg_conv_model.add(Dropout(0.5))
vgg_conv_model.add(Dense(4096, activation='relu'))
vgg_conv_model.add(Dropout(0.5))
vgg_conv_model.add(Dense(2, activation='softmax'))
print ('Summary of the model...')
vgg_conv_model.summary()
print ('Compiling model...')
#vgg_conv_model.compile(optimizer='sgd', loss='categorical_crossentropy', metrics=['accuracy'])
#vgg_conv_model.compile(optimizer=SGD(lr=1e-3), loss='categorical_crossentropy', metrics=['accuracy'])
vgg_conv_model.compile(optimizer=SGD(lr=1e-3, decay=1e-6, momentum=0.9, nesterov=True), loss='categorical_crossentropy', metrics=['accuracy'])
...
checkpoint = ModelCheckpoint('vgg_conv_model.h5',monitor='val_loss', verbose=1, save_best_only=True,mode='auto')
...
fitted_vgg_conv_model=vgg_conv_model.fit_generator(train_generator,
...
acc = fitted_vgg_conv_model.history['acc']
val_acc = fitted_vgg_conv_model.history['val_acc']
loss = fitted_vgg_conv_model.history['loss']
val_loss =fitted_vgg_conv_model.history['val_loss']
...
pred=vgg_conv_model.predict_generator(test_generator,steps=math.ceil(test_generator.samples/test_generator.batch_size),verbose=1)
