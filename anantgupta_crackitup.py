...
for epoch in range(epochRange):
    ...
    if(epoch % 10==0):
        ...
        totalTestLoss=0
        for testIndex in range(int(testX.shape[0]/batchSize)):
            testInput=Variable(torch.from_numpy(testX[batchSize*testIndex:batchSize*(testIndex + 1),:,:,:]))
            testLabels=Variable(torch.from_numpy(testY[batchSize*testIndex:batchSize*(testIndex + 1)]))
            totalTestLoss=totalTestLoss + criterion(model(testInput), testLabels)
